...
static Fql * pack_0 (Fq oil_u[QRUOV_m], Fql oil[QRUOV_M]) {
  for(int i=0; i<QRUOV_M; i++){
#if QRUOV_L == 3
    oil[i] = Fq2Fql(oil_u[i*QRUOV_L], oil_u[i*QRUOV_L+1], oil_u[i*QRUOV_L+2]) ;
#elif QRUOV_L == 10
    uint16_t a[QRUOV_L] ;
    for(int j=0; j<QRUOV_L; j++) a[j] = oil_u[i*QRUOV_L+j] ;
    oil[i] = Fq2Fql(a) ;
#else
