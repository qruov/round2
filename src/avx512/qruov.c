...
static inline UINT128_T load128(const uint8_t src [16]) {
  return * (UINT128_T *) src ;
}
static inline void save128(const UINT128_T s, uint8_t dst[16]) {
...
static inline UINT128_T load128(const uint8_t src [16]) {
  uint8_t dst[16] ALIGN_128BIT ;
  UINT128_T * r = (UINT128_T *) dst ;
  dst[ 0] = src[15] ;
  dst[ 1] = src[14] ;
  dst[ 2] = src[13] ;
  dst[ 3] = src[12] ;
...
static void ExpandVectorV (
  ...
  int i ;
  uint8_t * s = src ;
  for(i=0;i<QRUOV_V;i++) for(int k=0;k<QRUOV_L;k++) A[k][i] = *s++ ;
  for(int k=0;k<QRUOV_L;k++) memset(A[k]+QRUOV_V, 0, BLOCK_ALIGNED_BYTES(QRUOV_V)-QRUOV_V) ;
}
...
static void ExpandMatrixVxM (
  ...
  for(int i=0;i<QRUOV_V;i++){
    ...
    for(int k=0;k<QRUOV_L;k++) memset(A[i][k]+QRUOV_M, 0, BLOCK_ALIGNED_BYTES(QRUOV_M)-QRUOV_M) ;
  }
}
...
static void ExpandSymmetricMatrixVxV (
  ...
  for(int i=0; i<QRUOV_V; i++){
    ...
    for(int k=0;k<QRUOV_L;k++) memset(A[i][k]+QRUOV_V, 0, BLOCK_ALIGNED_BYTES(QRUOV_V)-QRUOV_V) ;
  }
}
...
//    VECTOR_V_MUL_MATRIX_VxV(yT, Pi1, yT_Pi1) ;
	...
	Fq y_j     [QRUOV_L] ;
        ...
        c_i += (uint64_t) Fql2Fq(Fql_mul(Fq2Fql_uint8_t_star(yT_Fi1_j), Fq2Fql_uint8_t_star(y_j)), QRUOV_perm(0)) ; // <-- shrink
      }
      c[i] = (Fq)(c_i % QRUOV_q) ;
    }
  }
...
int QRUOV_Verify(
  ...
  for(int i=0; i<QRUOV_V; i++,y_++)
    for(int k=0; k<QRUOV_L; k++)
      y[k][i] = Fql2Fq(*y_,k) ;
  for(int k=0; k<QRUOV_L; k++) memset(y[k]+QRUOV_V, 0, BLOCK_ALIGNED_BYTES(QRUOV_V)-QRUOV_V) ;
  

  for(int i=0; i<QRUOV_M; i++,oil_++)
    for(int k=0; k<QRUOV_L; k++)
      oil[k][i] = Fql2Fq(*oil_,k) ;
  for(int k=0; k<QRUOV_L; k++) memset(oil[k]+QRUOV_M, 0, BLOCK_ALIGNED_BYTES(QRUOV_M)-QRUOV_M) ;
