...
inline static int64_t vector_M_dot_vector_M_avx2 (const vector_V A, const vector_V B){
  Fq_vec * a = (Fq_vec *) A ;
  Fq_vec * b = (Fq_vec *) B ;
  Fq_vec   c ; c = _mm256_set1_epi32(0) ;
...
#if 1
#  define  vector_M_dot_vector_M  vector_M_dot_vector_M_avx2
#else
inline static int64_t vector_M_dot_vector_M (const vector_M A, const vector_M B){
  ...
  int64_t  C2 = vector_M_dot_vector_M_avx2 (A, B) ;
  if(C != C2){
    PRINT_vector_M(A);
    PRINT_vector_M(B);
    printf("C = ") ; printf("%d", C ) ; printf("\n") ;
    printf("C2= ") ; printf("%d", C2) ; printf("\n") ;
  }
  return C ;
}
#endif
